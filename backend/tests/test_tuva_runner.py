...
def test_build_dbt_command_run():
    ...
    assert cmd == ["dbt", "run", "--project-dir", "/fake/path", "--profiles-dir", "/fake/path"]
...
def test_build_dbt_command_with_select():
    ...
    assert cmd == ["dbt", "run", "--project-dir", "/fake/path", "--profiles-dir", "/fake/path", "--select", "cms_hcc"]
...
def test_build_dbt_command_seed():
    ...
    assert cmd == ["dbt", "seed", "--project-dir", "/fake/path", "--profiles-dir", "/fake/path"]
...
def test_build_dbt_command_build():
    ...
    assert cmd == ["dbt", "build", "--project-dir", "/fake/path", "--profiles-dir", "/fake/path"]
