...
class PriorAuth(Base, TimestampMixin):
    ...
    auth_start_date: Mapped[date | None]
    auth_end_date: Mapped[date | None]

    # Decision
    urgency: Mapped[str] = mapped_column(String(20), default="standard")  # "urgent", "standard"
    status: Mapped[str] = mapped_column(String(20), default="pending")  # "pending", "approved", "denied", "partial", "appealed", "withdrawn"
    decision: Mapped[str | None] = mapped_column(String(20))
    approved_units: Mapped[int | None]
    denial_reason: Mapped[str | None] = mapped_column(Text)

    # Appeal
    appeal_date: Mapped[date | None]
    appeal_status: Mapped[str | None] = mapped_column(String(20))
    peer_to_peer_date: Mapped[date | None]

    # Compliance tracking
    turnaround_hours: Mapped[int | None]  # calculated from request to decision
    compliant: Mapped[bool | None]  # within CMS timeframes?

    reviewer_id: Mapped[int | None]
